...
// Create -> uRPG Quest. Existing quests can be found in the Resources folder
using UnityEngine;
...
public abstract class ScriptableQuest : ScriptableObject
{
    ...
    // events to hook into /////////////////////////////////////////////////////
    public virtual void OnKilled(GameObject player, int questIndex, GameObject victim) {}
    public virtual void OnLocation(GameObject player, int questIndex, Collider location) {}

    // fulfillment /////////////////////////////////////////////////////////////
    // we pass the Quest instead of an index for ease of use and because we are
    // read-only here anyway
    public abstract bool IsFulfilled(GameObject player, Quest quest);

    // OnComplete is called when the quest is completed at the npc.
    // -> can be used to remove quest items from the inventory, etc.
    public virtual void OnCompleted(GameObject player, Quest quest) {}
    ...
    * {REWARDITEM}
    ...
    {STATUS}
    */
    public virtual string ToolTip(GameObject player, Quest quest)
    {
    ...
    static Dictionary<string, ScriptableQuest> cache;
    public static Dictionary<string, ScriptableQuest> dict
    {
        get
        {
            // not loaded yet?
            if (cache == null)
            {
                ...
                {
                    cache = quests.ToDictionary(quest => quest.name, quest => quest);
                }
                else
                {
